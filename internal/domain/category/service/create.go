...
func (s Service) CreateCategory(ctx context.Context,req param.CreateCategoryRequest,userID uint)(param.CreateCategoryResponse,error){
	...
	s.logger.Info("Creating category","user_id",userID,"name",req.Name,"type",req.Type)
	...
	s.logger.Info("Category created successfully","category_id",createdCategory.ID,"user_id",userID)
