...
func (s Service) DeleteCategory(ctx context.Context,categoryID,userID uint)(param.DeleteCategoryResponse,error){
	...
	logger.Info("Deleting category","category_id",categoryID,"user_id",userID)
	...
	if err !=nil{
		logger.Error("Failed to check category transactions","category_id",categoryID,"error",err.Error())
	...
	if hasTransaction {
		logger.Warn("Attempted to delete category with transactions","category_id",categoryID,"user_id",userID)
