...
func (s Service) ParseToken(tokenStr string) (*Claims, error) {
	...
	token,err:=jwt.ParseWithClaims(tokenStr,&Claims{},func(token *jwt.Token)(any,error){
		if _,ok:=token.Method.(*jwt.SigningMethodHMAC);!ok{
			return nil,richerror.New(OpParseToken).WithMessage("invalid siging method").WithKind(richerror.KindInvalid).WithMeta("algorithm",token.Header["alg"])
		}
		return  key,nil
	})
	if err !=nil {
...
func (s Service) ParseRefreshToken(refreshToken string) (*Claims,error){
	...
	if claims.Subject !=s.config.RefreshSubject {
		return  nil,richerror.New(OpParseToken).WithMessage("invaid refresh token subject").WithKind(richerror.KindInvalid)
	}
	return  claims,nil
}